...
childAppend(el, index, parent) {
    ...
    for (var u = 0; u < attrsKeys.length; u++) {
        if (typeof attrs[attrsKeys[u]] == 'undefined' && attrsKeys[u].startsWith(':'))
            attrs[attrsKeys[u]] = []
        if (typeof attrs[attrsKeys[u]] == 'undefined')
            attrs[attrsKeys[u]] = ''

        if (attrsKeys[u].startsWith(':')) {
            attrs[attrsKeys[u]].push(el.getAttribute(attrsKeys[u]))
...
renderChild(el) {
    if (el.type == 'tag') {
        ...
        for (const [k, v] of Object.entries(el.attrs)) {
            if (k.startsWith(':')) {
                if (element.getAttribute(k.slice(1)) == null) 
                    element.setAttribute(k.slice(1), eval(`${v}`));
                else 
                    element.setAttribute(k.slice(1), element.getAttribute(k.slice(1))+' '+eval(`${v}`));
            } else {
                if (element.getAttribute(k) == null) 
